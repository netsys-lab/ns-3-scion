...
NS_LOG_COMPONENT_DEFINE("SCIONQueueDiscItem");

SCIONQueueDiscItem::SCIONQueueDiscItem(Ptr<Packet> p,
                                     const Address& addr,
                                     uint16_t protocol,
...
SCIONQueueDiscItem::~SCIONQueueDiscItem()
{
    NS_LOG_FUNCTION(this);
}

uint32_t
SCIONQueueDiscItem::GetSize() const
{
    NS_LOG_FUNCTION(this);
...
const SCIONHeader&
SCIONQueueDiscItem::GetHeader() const
{
    return m_header;
}

void
SCIONQueueDiscItem::AddHeader()
{
    NS_LOG_FUNCTION(this);
...
void
SCIONQueueDiscItem::Print(std::ostream& os) const
{
    if (!m_headerAdded)
    {
        os << m_header << " ";
...
bool
SCIONQueueDiscItem::Mark()
{
    NS_LOG_FUNCTION(this);
    if (!m_headerAdded && m_header.GetEcn() != SCIONHeader::ECN_NotECT)
    {
        m_header.SetEcn(SCIONHeader::ECN_CE);
        return true;
    }
    return false;
}
...
bool
SCIONQueueDiscItem::GetUint8Value(QueueItem::Uint8Values field, uint8_t& value) const
{
    bool ret = false;
    ...
    {
    case IP_DSFIELD:
        value = m_header.GetTrafficClass(); // or TypeOfService (ToS)
        ret = true;
        break;
    }
...
uint32_t
SCIONQueueDiscItem::Hash(uint32_t perturbation) const
{
